from flask import Markup, Blueprint, current_app, url_for, request
...
class Share(object):
    ...
    def init_app(self, app):
        ...
        app.config.setdefault('SHARE_SITES', 'weibo, wechat, douban, '
                                             'facebook, twitter, google, linkedin, qq, qzone')
    ...
    def load(css_url=None, js_url=None):
        if current_app.config['SHARE_SERVE_LOCAL']:
            ...
            js_url = url_for('share.static', filename='js/share.min.css')
        if css_url is None:
    ...
    def create(title='', sites=None, mobile_sites=None, align='left', addition_class=''):
        if current_app.config['SHARE_HIDE_ON_MOBILE']:
            ...
            if m is not None:
                return ''
        if sites is None:
            sites = current_app['SHARE_SITES']
        if mobile_sites is None:
            mobile_sites = current_app['SHARE_MOBILE_SITES']
        return Markup('''<div class="social-share %s" date-sites="%s" data-mobile-sites="%s"
